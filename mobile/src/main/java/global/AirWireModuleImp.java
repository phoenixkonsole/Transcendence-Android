...
import global.airwire.DefaultCoinSelector;
...
public class AirWireModuleImp implements AirWireModule {

    private static final Logger logger = LoggerFactory.getLogger(AirWireModuleImp.class);
    ...
    public AirWireModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration,ContactsStoreDao contactsStore,RateDbDao rateDb,BackupHelper backupHelper) {
...
//    }
    ...
    public Transaction completeTxWithCustomFee(Transaction transaction,Coin fee) throws InsufficientMoneyException{
        ...
        walletManager.completeSend(sendRequest);
        // if the fee is different to the custom fee and the tx size is lower than 1kb (1000 bytes in airwire core)
...
//                    }
    ...
    public AirWireRate getRate(String coin) {
   ...
   public boolean isSyncWithNode() throws NoPeerConnectedException {
    ...
    @Override
    public List<AirWireRate> listRates() {
        return rateDb.list();
    }
    ...
    public void saveRate(AirWireRate airwireRate){
        rateDb.insertOrUpdateIfExist(airwireRate);
    }
