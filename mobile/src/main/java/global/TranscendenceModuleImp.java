...
import global.transcendence.DefaultCoinSelector;
...
public class TranscendenceModuleImp implements TranscendenceModule {

    private static final Logger logger = LoggerFactory.getLogger(TranscendenceModuleImp.class);
    ...
    public TranscendenceModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration,ContactsStoreDao contactsStore,RateDbDao rateDb,BackupHelper backupHelper) {
...
//    }
    ...
    public Transaction completeTxWithCustomFee(Transaction transaction,Coin fee) throws InsufficientMoneyException{
        ...
        walletManager.completeSend(sendRequest);
        // if the fee is different to the custom fee and the tx size is lower than 1kb (1000 bytes in transcendence core)
...
//                    }
    ...
    public TranscendenceRate getRate(String coin) {
   ...
   public boolean isSyncWithNode() throws NoPeerConnectedException {
    ...
    @Override
    public List<TranscendenceRate> listRates() {
        return rateDb.list();
    }
    ...
    public void saveRate(TranscendenceRate transcendenceRate){
        rateDb.insertOrUpdateIfExist(transcendenceRate);
    }
