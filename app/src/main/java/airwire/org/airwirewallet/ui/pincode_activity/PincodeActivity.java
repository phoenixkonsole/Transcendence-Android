package airwire.org.airwirewallet.ui.pincode_activity;
...
import airwire.org.airwirewallet.ui.backup_mnemonic_activity.MnemonicActivity;
import airwire.org.airwirewallet.ui.base.BaseActivity;
import airwire.org.airwirewallet.ui.settings_pincode_activity.KeyboardFragment;
import airwire.org.airwirewallet.ui.start_activity.StartActivity;

import static airwire.org.airwirewallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (airwireApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (airwireApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            airwireApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        airwireApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(airwireApplication.getAppConf().getPincode().equals(pincode)){
