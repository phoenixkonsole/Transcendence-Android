...
import airwire.org.airwirewallet.ui.contacts_activity.ContactsActivity;
import airwire.org.airwirewallet.ui.donate.DonateActivity;
import airwire.org.airwirewallet.ui.settings_activity.SettingsActivity;
import airwire.org.airwirewallet.ui.wallet_activity.WalletActivity;

import static airwire.org.airwirewallet.module.AirWireContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends AirWireActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = airwireApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = airwireModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (airwireModule.getChainHeight()+10>peerHeight) {
            ...
            if (airwireModule.isAnyPeerConnected()) {
                long peerHeight = airwireModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (airwireModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
