...
import airwire.org.airwirewallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static airwire.org.airwirewallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static airwire.org.airwirewallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static airwire.org.airwirewallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = airwireModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (airwireRate!=null) {
                    localCurrency = airwireApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * airwireRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + airwireRate.getCode();
                ...
                holder.title.setText(getAddressOrContact(airwireModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(airwireModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(airwireModule.getConf().getNetworkParams()).toBase58());
                }*/
