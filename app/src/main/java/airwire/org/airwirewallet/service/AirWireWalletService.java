package airwire.org.airwirewallet.service;
...
import org.airwirej.core.listeners.AbstractPeerDataEventListener;
...
import airwire.org.airwirewallet.module.store.SnappyBlockchainStore;
import airwire.org.airwirewallet.rate.CoinMarketCapApiClient;
import airwire.org.airwirewallet.rate.RequestAirWireRateException;
...
import static airwire.org.airwirewallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static airwire.org.airwirewallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static airwire.org.airwirewallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
...
import static airwire.org.airwirewallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
...
import static airwire.org.airwirewallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static airwire.org.airwirewallet.service.IntentsConstants.NOT_COINS_RECEIVED;

/**
 * Created by akshaynexus on 6/12/17.
 */

public class AirWireWalletService extends Service{

    private Logger log = LoggerFactory.getLogger(AirWireWalletService.class);

    private AirWireApplication airwireApplication;
    private AirWireModuleImp module;
...
//    NotificationManager mNotificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
    ...
    public class AirWireBinder extends Binder {
        public AirWireWalletService getService() {
            return AirWireWalletService.this;
        }
    }
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (AirWireContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    airwireApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.airwirej.core.Context.propagate(AirWireContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.airwirej.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), AirWireWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(AirWireWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                        ...
                        mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(AirWireWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("AirWire service started");
            // Android stuff
            ...
            module = (AirWireModuleImp) airwireApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to pivtrum node
            /*pivtrumPeergroup = new PivtrumPeergroup(airwireApplication.getNetworkConf());
            ...
            String filename = AirWireContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file,filename).exists();
            blockchainStore = new SnappyBlockchainStore(AirWireContext.CONTEXT,file,filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("AirWire service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void requestRateCoin(){
        final AppConf appConf = airwireApplication.getAppConf();
        AirWireRate airwireRate = module.getRate(appConf.getSelectedRateCoin());
        if (airwireRate == null || airwireRate.getTimestamp() + AirWireContext.RATE_UPDATE_TIME < System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        CoinMarketCapApiClient.AirWireMarket airwireMarket = c.getAirWirePxrice();
                        AirWireRate airwireRate = new AirWireRate("USD",airwireMarket.priceUsd,System.currentTimeMillis());
                        module.saveRate(airwireRate);
                        final AirWireRate airwireBtcRate = new AirWireRate("BTC",airwireMarket.priceBtc,System.currentTimeMillis());
                        module.saveRate(airwireBtcRate);

                        // Get the rest of the rates:
                        List<AirWireRate> rates = new CoinMarketCapApiClient.BitPayApi().getRates(new CoinMarketCapApiClient.BitPayApi.RatesConvertor<AirWireRate>() {
                            @Override
                            public AirWireRate convertRate(String code, String name, BigDecimal bitcoinRate) {
                                BigDecimal rate = bitcoinRate.multiply(airwireBtcRate.getRate());
                                return new AirWireRate(code,rate,System.currentTimeMillis());
                            }
                        });

                        for (AirWireRate rate : rates) {
                            module.saveRate(rate);
                        }

                    } catch (RequestAirWireRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
