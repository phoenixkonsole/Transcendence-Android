package airwire.org.airwirewallet.rate.db;
...
import airwire.org.airwirewallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<AirWireRate> implements RateDbDao<AirWireRate> {
    ...
    @Override
    protected ContentValues buildContent(AirWireRate obj) {
    ...
    @Override
    protected AirWireRate buildFrom(Cursor cursor) {
        ...
        return new AirWireRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(AirWireRate airwireRate) {
        if (getRate(airwireRate.getCode())==null){
            insert(airwireRate);
        }else {
            updateByKey(KEY_COIN,airwireRate.getCode(),airwireRate);
        }
    }
}
