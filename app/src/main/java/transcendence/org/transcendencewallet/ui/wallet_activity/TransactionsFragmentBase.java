...
import transcendence.org.transcendencewallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static transcendence.org.transcendencewallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static transcendence.org.transcendencewallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static transcendence.org.transcendencewallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = transcendenceModule.listTx();
        ...
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (transcendenceRate!=null) {
                    localCurrency = transcendenceApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * transcendenceRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                ...
                holder.title.setText(getAddressOrContact(transcendenceModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(transcendenceModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(transcendenceModule.getConf().getNetworkParams()).toBase58());
                }*/
