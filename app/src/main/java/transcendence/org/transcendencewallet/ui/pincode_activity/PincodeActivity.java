package transcendence.org.transcendencewallet.ui.pincode_activity;
...
import transcendence.org.transcendencewallet.ui.settings_pincode_activity.KeyboardFragment;
import transcendence.org.transcendencewallet.ui.start_activity.StartActivity;

import static transcendence.org.transcendencewallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (transcendenceApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        transcendenceApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(transcendenceApplication.getAppConf().getPincode().equals(pincode)){
