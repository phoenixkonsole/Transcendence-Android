...
import transcendence.org.transcendencewallet.ui.contacts_activity.ContactsActivity;
import transcendence.org.transcendencewallet.ui.donate.DonateActivity;
import transcendence.org.transcendencewallet.ui.settings_activity.SettingsActivity;
import transcendence.org.transcendencewallet.ui.wallet_activity.WalletActivity;

import static transcendence.org.transcendencewallet.module.TranscendenceContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends TranscendenceActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = transcendenceApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = transcendenceModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (transcendenceModule.getChainHeight()+10>peerHeight) {
            ...
            if (transcendenceModule.isAnyPeerConnected()) {
                long peerHeight = transcendenceModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (transcendenceModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
