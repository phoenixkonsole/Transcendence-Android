...
import org.transcendencej.core.listeners.AbstractPeerDataEventListener;
...
import transcendence.org.transcendencewallet.module.store.SnappyBlockchainStore;
import transcendence.org.transcendencewallet.rate.CoinMarketCapApiClient;
import transcendence.org.transcendencewallet.rate.RequestTranscendenceRateException;
...
import static transcendence.org.transcendencewallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static transcendence.org.transcendencewallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static transcendence.org.transcendencewallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
...
import static transcendence.org.transcendencewallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
...
import static transcendence.org.transcendencewallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static transcendence.org.transcendencewallet.service.IntentsConstants.NOT_COINS_RECEIVED;
...
//    NotificationManager mNotificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
    ...
    public class TranscendenceBinder extends Binder {
        public TranscendenceWalletService getService() {
            return TranscendenceWalletService.this;
        }
    }
    ...
    public IBinder onBind(Intent intent) {
        return new TranscendenceBinder();
    }
    ...
    private final PeerDataEventListener blockchainDownloadListener = new AbstractPeerDataEventListener() {
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (TranscendenceContext.IS_TEST)
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.transcendencej.core.Context.propagate(TranscendenceContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.transcendencej.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), TranscendenceWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(TranscendenceWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                        ...
                        mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(TranscendenceWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("Transcendence service started");
            // Android stuff
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("Transcendence service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void requestRateCoin(){
        final AppConf appConf = transcendenceApplication.getAppConf();
        TranscendenceRate transcendenceRate = module.getRate(appConf.getSelectedRateCoin());
        if (transcendenceRate == null || transcendenceRate.getTimestamp() + TranscendenceContext.RATE_UPDATE_TIME < System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        CoinMarketCapApiClient.TranscendenceMarket transcendenceMarket = c.getTranscendencePxrice();
                        TranscendenceRate transcendenceRate = new TranscendenceRate("USD",transcendenceMarket.priceUsd,System.currentTimeMillis());
                        module.saveRate(transcendenceRate);
                        final TranscendenceRate transcendenceBtcRate = new TranscendenceRate("BTC",transcendenceMarket.priceBtc,System.currentTimeMillis());
                        module.saveRate(transcendenceBtcRate);

                        // Get the rest of the rates:
                        List<TranscendenceRate> rates = new CoinMarketCapApiClient.BitPayApi().getRates(new CoinMarketCapApiClient.BitPayApi.RatesConvertor<TranscendenceRate>() {
                            @Override
                            public TranscendenceRate convertRate(String code, String name, BigDecimal bitcoinRate) {
                                BigDecimal rate = bitcoinRate.multiply(transcendenceBtcRate.getRate());
                                return new TranscendenceRate(code,rate,System.currentTimeMillis());
                            }
                        });

                        for (TranscendenceRate rate : rates) {
                            module.saveRate(rate);
                        }

                    } catch (RequestTranscendenceRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
