package transcendence.org.transcendencewallet.rate.db;
...
import transcendence.org.transcendencewallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<TranscendenceRate> implements RateDbDao<TranscendenceRate> {
    ...
    @Override
    protected ContentValues buildContent(TranscendenceRate obj) {
    ...
    @Override
    protected TranscendenceRate buildFrom(Cursor cursor) {
        ...
        return new TranscendenceRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(TranscendenceRate transcendenceRate) {
        if (getRate(transcendenceRate.getCode())==null){
            insert(transcendenceRate);
        }else {
            updateByKey(KEY_COIN,transcendenceRate.getCode(),transcendenceRate);
        }
    }
}
